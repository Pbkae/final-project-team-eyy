...
# @st.cache_data
# def load_data():
#     # Replace with your actual data loading/creation logic
#     # Example: Load from a CSV or generate a DataFrame
#     df = pd.read_csv("cleaned_survey_results.csv")  # Replace with actual data source
#     df.to_parquet('cleaned_survey_results.parquet', engine='pyarrow', index=False)
#     return df
...
if section == "Data Preparation":
    ...
    # remaining_columns = high_missing_cols.index
    ...
    # st.write("Numerical Columns Missing Values:", data[numerical_cols].isnull().sum().sum())
    # st.write("Categorical Columns Missing Values:", data[categorical_cols].isnull().sum().sum())
    ...
    # data.to_csv('cleaned_survey_results.parquet', index=False)

    
    # Show the first few rows of the original dataset
    st.subheader("Original Dataset Preview")
    st.image("original-dataset-preview.png", use_column_width=True)

    # # Display column information
    st.subheader("Column Information")
    st.image("column-information.png", use_column_width=False, width=400)  # Adjust the width as needed


    # Show remaining columns after cleaning
    st.subheader("Remaining Columns After Cleaning")
    st.write("Numerical Columns Missing Values: 0")
    st.write("Categorical Columns Missing Values: 0")

    # Display the cleaned data preview
    st.subheader("Cleaned Dataset Preview")
    st.image("cleaned-dataset-preview.png", use_column_width=True)
...
if section == "K-means Clustering":
    ...
    st.image("elbow-method.png", use_column_width=True)
    ...
    # data_imputed.to_csv('clustered_survey_data.parquet', index=False)
    st.image("cluster-results.png", use_column_width=True)
...
if section == "Linear Regression":
    ...
    #     st.image("influential-features.png", use_column_width=True)
    ...
    st.subheader("Model Evaluation")
    st.write("""
        Mean Squared Error (MSE): .00
    """)
...
if section == "Descriptive Statistics & Visualization":
    ...
    st.write("Displaying first few rows of the dataset:")
    st.image("first-row-dataset.png", use_column_width=True)
    ...
    # plt.tight_layout()
    # st.pyplot(plt)
    st.image("years-coding.png", use_column_width=True)
    ...
    # plt.grid(True)
    # st.pyplot(plt)
    st.image("age-by-respondents.png", use_column_width=True)
    ...
    # st.pyplot(plt)
    st.image("age-by-countries.png", use_column_width=True)
    ...
    # plt.grid(axis='y')
    # st.pyplot(plt)
    st.image("salary-by-education.png", use_column_width=True)
    ...
    # data = data[(data['CompTotal'] <= q99) & (data['CompTotal'] > 0)].dropna(subset=['YearsCodePro', 'CompTotal'])
    # plt.figure(figsize=(10, 6))
    # sns.scatterplot(data=data, x='YearsCodePro', y='CompTotal', alpha=0.6)
    # sns.regplot(data=data, x='YearsCodePro', y='CompTotal', scatter=False, color='red')
    ...
    st.image("scatter-plot.png", use_column_width=True)
    ...
    # q99 = data['CompTotal'].quantile(0.99)  # 99th percentile
    # df_cleaned = data[(data['CompTotal'] <= q99) & (data['CompTotal'] > 0)].dropna(subset=['YearsCodePro', 'CompTotal'])

    # # Scatter plot with trend line
    # plt.figure(figsize=(10, 6))
    # sns.scatterplot(data2=df_cleaned, x='YearsCodePro', y='CompTotal', alpha=0.6)
    # sns.regplot(data2=df_cleaned, x='YearsCodePro', y='CompTotal', scatter=False, color='red')
    ...
    # plt.grid(True)
    # plt.show()
    st.image("eda-heatmap.png", use_column_width=True)
...
if section == "Apriori Algorithm":
    ...
    # st.table(top_rules)
    st.image("apriori_results_1.png", use_column_width=True)
    ...
    # st.table(top_rules)
    st.image("apriori_results_2.png", use_column_width=True)
